...
export const formatDate = (date: Date): string => {
  const weekday = new Intl.DateTimeFormat('es-ec', { weekday: 'long' }).format(
    date
  );
...
export const filterHoursGreaterThanCurrentTime = (
  hours: SelectItemGroup[]
    ...
    .map((group) => {
      const filteredItems = group.items.filter((item) => {
        const itemTime =
          ...
          parseInt(item.value.split(':')[1]); // Convertir la hora del item a minutos
...
export const formatForChart = (
  input: { rol: string; total: number }[]
): { roles: string[]; data: number[] } => {
...
export const formatDataByDia = (
  input: { rol: string; dia: string; total: number }[]
): { dias: string[]; datas: { label: string; data: number[] }[] } => {
...
export const formatAgendamientoData = (
  input: { dia: string; total: number }[]
): { labels: string[]; data: { label: string; data: number[] }[] } => {
  // Extraer los días únicos
...
export const transformAsistioData = (
  data: { asistio: string; total: number }[]
